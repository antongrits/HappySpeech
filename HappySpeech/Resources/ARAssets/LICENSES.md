...
## Статус каталога (ADR-V30-MASCOT-2D, 2026-05-19)

Каталог `ARAssets/` **больше не содержит USDZ-моделей**.

Ранее здесь лежали:

- `lyalya3d.usdz` / `lyalya3d_v2.usdz` / `lyalya3d_v3.usdz` — 3D-модели маскота «Ляля»;
...
1. **3D-маскот окончательно выведен из рендера** (ADR-V30-MASCOT-2D, сменяет
   ADR-V29-MASCOT-3D). Процедурная USDZ-модель `lyalya3d_v3.usdz` выглядела
   непрофессионально (примитивы-сферы без риггинга) и вызывала видимый
   2D/3D-«мигающий» переход при асинхронной загрузке. Приложение использует
   единый профессионально анимированный 2D-канон «подружка-пчёлка» — иллюстрации
   `mascot_lyalya_*` в `Assets.xcassets`, анимированные процедурно в SwiftUI
   (`HSMascotView`).
