...
**Маскот:** Ляля — бабочка-подружка  
**Голос:** характерный голос персонажа `lyalya-character-v1` (см. ниже)  
**Формат:** M4A (AAC, 16 kHz mono, 32 kbps)  
**Нормализация:** -16 LUFS, true peak ≤ -1.5 dBFS  
**Лицензия:** Microsoft Edge TTS — royalty-free для коммерческого использования  

## Голос персонажа `lyalya-character-v1` (обновлено 2026-05-17)

Раньше озвучка использовала «чистый» нейронный голос `ru-RU-SvetlanaNeural` —
он звучал как генеричный взрослый диктор, а не как голос мультяшного
персонажа. Новый профиль делает голос тёплым, живым и чуть «мультяшным»,
сохраняя при этом полную разборчивость (критично для логопедии).

Профиль голоса:
- База: Microsoft Edge TTS `ru-RU-SvetlanaNeural`
- Интонация: `rate +8%`, `pitch +25Hz` (бодрее и выше базового)
- Обработка ffmpeg: pitch-shift +6 % (`asetrate` + `atempo`-компенсация
  длительности), мягкая компрессия `acompressor`, `loudnorm I=-16`
- Результат — характерный детский дружелюбный тембр без «писка бурундука»

Почему edge-tts, а не Silero/Piper: edge-tts даёт самое естественное
русское нейронное произношение и не требует тяжёлых зависимостей
(torch ~2.5 GB). Для русского edge-tts отдаёт только 2 голоса
(Svetlana / Dmitry), поэтому характерность достигается аудио-обработкой.

## Очистка служебных пометок (обновлено 2026-05-17)

Из ОЗВУЧИВАЕМОГО текста убраны методические пометки, которые ребёнок
слышать не должен (34 фразы):
- `(ударение на ...)` — пометка для логопеда/экрана → удалена (озвучка
  только слова: `белянка (ударение на ля)` → «белянка»)
- падежные маркеры `(куда)`, `(где)`, `(откуда)`, `(о ком)` и т.п. →
  заменены паузой между контрастными фразами
  (`иду в школу (куда) нахожусь в школе (где)` → «иду в школу. нахожусь
  в школе»)
- варианты окончаний `(а)` → берётся базовая форма (`я дал(а)` → «я дал»)
- слово-подсказка в скобках (`я вижу (кошка)`) → озвучивается, скобки
  снимаются (это форма-подсказка для грамматической игры)

Текст для ЭКРАНА (поле `word` в контент-паках) не меняется — методисту
и ребёнку визуальные пометки нужны. Меняется только TTS-текст.

## Генерация

Скрипты в `~/Downloads/HappySpeech/_workshop/scripts/` (в репо не коммитятся):

```bash
pip install edge-tts
brew install ffmpeg

# 1. Очистка служебных пометок -> lyalya-tts-overrides.json
python3 clean_phrase_texts.py

# 2. Перегенерация озвучки характерным голосом (фоном, возобновляемо)
python3 regenerate_lyalya_voice.py
python3 regenerate_lyalya_voice.py --status   # прогресс
```

Скрипт `regenerate_lyalya_voice.py`:
1. Генерирует MP3 через edge-tts (rate +8%, pitch +25Hz)
2. Применяет ffmpeg pitch-shift + компрессию + loudnorm -16 LUFS
3. Конвертирует в M4A (AAC 16 kHz mono 32 kbps)
4. Ведёт manifest — идемпотентность и возобновляемость
5. Удаляет промежуточные MP3 сразу после конвертации

**Известное ограничение:** регенерации поддаются 1558 файлов, для которых
исходный текст хранится в `lyalya-phrase-mapping.json`. Для остальных
~12 000 файлов (сгенерированы скриптом v18, не сохранённым в git)
исходные тексты утеряны — их регенерация требует предварительной
Whisper-транскрипции существующего аудио (отдельный этап).
