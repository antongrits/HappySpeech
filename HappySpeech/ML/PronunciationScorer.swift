...
/// своём наборе звуков.
///
/// > Important: Метрики ниже получены на **синтетическом** датасете и **не
/// > валидированы на реальной детской речи**. Это accuracy на held-out синтетике,
/// > а не клиническая точность распознавания произношения у детей. Значения
/// > служат лишь для сравнения моделей между собой.
///
/// | Группа | Звуки | Accuracy на синтетике |
/// |--------|-------|---------|
/// | `.whistling` | С, З, Ц | ~95% |
/// | `.hissing` | Ш, Ж, Ч, Щ | ~100% |
/// | `.sonants` | Р, Л | ~93% |
