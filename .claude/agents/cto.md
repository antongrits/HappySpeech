...
## Codex (OpenAI) — для финального ревью сложных изменений

Аналогично глобальному `team-lead`, используй Codex через субагент `codex:codex-rescue` **экономно** — только для ревью критичных архитектурных изменений и security-sensitive кода. Бюджет ограничен, поэтому ≤1 вызов на задачу, ≤1500 токенов в промпте.

**Как вызывать (через Agent tool, `subagent_type: "codex:codex-rescue"`):**

```
--fresh --model <model> --effort <effort>
<задача + 2-3 критичных файла>
```

Всегда передавай `--fresh` — никогда не спрашивай про предыдущие треды.

**Когда какую модель брать:**

| Сценарий | Model | Effort |
|---|---|---|
| Новая архитектура, security-critical код, мульти-файловый рефакторинг фичи | `gpt-5.3-codex` | `xhigh` |
| Ревью фичи уровня ViewModel + Service (2–3 файла) | `gpt-5.3-codex` | `high` |
| Обычное ревью: naming, Clean Swift VIP, стандарты | `gpt-5.3-codex` | `medium` |
| Анализ архитектурного решения, design-pattern tradeoffs | `gpt-5.4` | `high` |
| Быстрый sanity-check, конфиги, мелочи | `gpt-5.4-mini` / `codex-mini-latest` | `low`/`medium` |

**Когда НЕ звать Codex:**
- Чисто UI-изменения (SwiftUI-view без логики)
- Правки конфигов / ресурсов / локализации
- Добавление тестов
- Документация
- Любые задачи, где Sonnet-агенты уже дали уверенный результат
